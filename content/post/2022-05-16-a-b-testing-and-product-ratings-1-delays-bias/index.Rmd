...
Suppose you're a data scientist at an e-commerce web site, responsible for
...
However, when you're making _changes_ to the site, your interventions
...
Imagine you run a web site that sells shoes, and you want to test
some sort of interface change. You may suspect that the change won't so much
affect how *many* items are purchased, but instead will affect more
*which* items are purchased, which in turn will affect *satisfaction*. 
...
Furthermore, different people with different experiences differ in their rating
behavior. Higher-priced items, where you spent a bunch of time reading
...
Let's do some simulations. We don't need to have
...
Here's some R code to simulate this.
...
and if having strong feelings make you more likely to rate, then ratings will
look more positive than the actual level of satisfaction.
* Early ratings will differ from late ratings. In this scenario they are
...
services with very loyal customers, such as Rent the Runway or StitchFix,
...
This will substantially increase the amount of data you'll need to collect.
...
the right decisions.
* An easy thing to do is simply to wait. Run the experiment longer, and wait
longer to get more ratings, before you make a decision.
* Look at rating rates. If the intervention made users much more or less likely
to rate during the window of your experiment, then the average ratings
...
different answer? 
* Thing about mechanisms. You may be tempted to A/B test an intervention
where you remind people to rate their purchases a week after they receive
their shoes. This test could easily yield a change in average ratings. But
...
These are not even all of the issues that arise with
